...
class TaskService {
    ...
    try {
      ...
      if (isNetworkOrCorsError) {
        ...
        if (error instanceof Error && error.message.includes('CORS')) {
          console.warn('CORS error detected - this may indicate that the backend is not properly configured to accept requests from this origin.');
          console.warn('Please check if the backend supports requests from http://localhost:3000');
        }

        // Attempt to wake up the backend
        try {
          const { backendWakeupService } = await import('../../lib/api/backend-wakeup');
          await backendWakeupService.ensureBackendAwake();

          // Retry the request after waking up the backend
        ...
        throw error; // Re-throw the original error if it's not a network/CORS error
      }
    }
  }
  ...
  async createTask(title: string, description?: string, priority: 'low' | 'medium' | 'high' = 'medium', due_date?: string): Promise<Task> {
    try {
      ...
      // Convert response to match Task interface
      const task = {
      ...
      // Import and use the utility function to ensure consistent formatting
...
const TaskManager = ({ userId }: { userId: number }) => {
  // Import the utility function to ensure consistent task formatting
  const formatTaskForEvent = (t: any) => ({
    id: String(t.id),
    title: t.title || '',
    description: t.description || '',
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Format the task consistently using the utility function
  ...
  const handleToggleTask = async (taskId: string | number) => {
    try {
      ...
      if (currentTask) {
        // Toggle the status - if it was completed, mark as incomplete, and vice versa
        ...
        // Dispatch a custom event to notify other parts of the app about the task update
        window.dispatchEvent(new CustomEvent('taskToggled', { detail: formattedTask }));
      } else {
        // If we can't find the task, just toggle assuming it's marking as complete
  ...
  const handleDeleteTask = async (taskId: string | number) => {
    ...
    try {
      ...
      // Dispatch a custom event to notify other parts of the app about the task deletion
...
export default function ChatBot() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (lowerInput.includes('list') && (lowerInput.includes('task') || lowerInput.includes('show'))) {
        ...
        // Handle task updates
        try {
          // Extract task title and what to update (simple extraction)
          const parts = input.split('to');
          if (parts.length >= 2) {
            const taskToUpdate = parts[0].replace(/(update|edit|change)/gi, '').trim();
            const updateValue = parts[1].trim();

            // For now, let's just search for a task with the matching title
            ...
            if (taskToModify) {
              // Update the task title for now (could be extended for other fields)
              ...
              // If we can't find the task, fall back to AI
            }
          }
        } catch (taskError) {
        ...
        try {
          ...
          // Search for a task with the matching title
          const allTasks = await taskService.getTasks();
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`fixed bottom-6 right-6 z-50 w-80 flex flex-col border border-white/20 rounded-[2rem] shadow-2xl bg-gradient-to-br from-[#0a0a0f] to-[#1a1a2e] text-white overflow-hidden transition-all duration-300 transform ${
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#1a1a2e]/30 to-[#2d1b69]/30 border-b border-white/10 backdrop-blur-xl bg-opacity-30 relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-center gap-3">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-[#8b5cf6]/40 relative overflow-hidden group">
              ...
              <div>
                ...
                <p className="text-xs text-white/60 font-bold uppercase tracking-widest flex items-center gap-1">
...
if (typeof document !== 'undefined') {
  ...
  styleElement.id = 'chatbot-animations';
  styleElement.innerHTML = styles;
  document.head.appendChild(styleElement);

  // Cleanup on component unmount (if needed in a different context)
  // This is just for reference since it's not a functional component
}